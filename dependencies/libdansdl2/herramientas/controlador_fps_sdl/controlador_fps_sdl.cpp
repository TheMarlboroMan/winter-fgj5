...
void Controlador_fps_SDL::iniciar_paso_loop()
{
	acumulador_delta+=obtener_delta();
	tiempo_paso=acumulador_delta;
	if(max_tiempo_paso && tiempo_paso > max_tiempo_paso) tiempo_paso=max_tiempo_paso;
	acumulador_delta=0.0;
}

bool Controlador_fps_SDL::consumir_loop(float delta)
{
	bool continuar=tiempo_paso >= delta;

	if(continuar) //Si continuamos en el loop, restamos otro pico.
	{
		tiempo_paso-=delta;
	}
	else //Si vamos a dejar el loop, guardamos lo que nos ha sobrado para la próxima.
	{
		acumulador_delta+=tiempo_paso;
	}
